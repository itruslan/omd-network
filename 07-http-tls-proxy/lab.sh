...
run_check() {
    ...
    if grep -q 'test is successful' <<<"${nginx_out}"; then
        ...
        # Проверка остановилась на сертификате, которого до `up` ещё нет, —
        # значит про остальную конфигурацию она ничего не сказала. Прежний
        # текст выдавал эту остановку за принятую конфигурацию.
        printf '[ note ] конфигурация проверена не полностью: nginx остановился\n'
        printf '         на сертификате, он появится при up\n'
    else
...
start_app() {
    ...
    echo $! > "${pidfile}"
    sleep 0.3
    # Фоновый запуск сам по себе ничего не доказывает: если приложение упало
    # при старте — например, порт уже занят, — `set -e` этого не заметит, и
    # скрипт сообщил бы об успехе. Тогда учебный отказ в практике оказался бы
    # не тем, что задумано, а поломкой стенда.
    if ! kill -0 "$(cat "${pidfile}")" 2>/dev/null; then
        printf 'Бэкенд %s не запустился:\n' "${letter}" >&2
        tail -5 "${logfile}" >&2
        rm -f "${pidfile}"
        return 1
    fi
