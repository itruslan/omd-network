#!/usr/bin/env python3
"""Сводка захвата в записи формата AWS VPC Flow Logs, версия 2.

Облака на стенде нет, поэтому журнал потоков не снят, а получен из
настоящего захвата на интерфейсе шлюза — так, как платформа свела бы те же
пакеты: одна запись на пятёрку «адреса, порты, протокол» в одном
направлении за интервал сведения в 60 секунд. `start` и `end` — время
первого и последнего пакета потока внутри интервала. Решение фильтров
облака на стенде принимать некому, поэтому `action` у всех записей ACCEPT.
