...
run_check() {
    ...
    # Таблицы читаются все: правило маршрутизации может направить трафик
    # стенда в таблицу, которой в main не видно.
    #
    ...
    local routes net
    if ! routes="$(ip -j -4 route show table all 2>/dev/null)" || [[ -z ${routes} ]]; then
